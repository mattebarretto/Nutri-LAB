...
def password_is_valid(request, password, confirm_password):
    ...
    if not re.search('[A-Z]', password):
        messages.add_message(request, constants.ERROR, 'A sua senha deve conter uma ou mais letras maíusculas')
        return False
    if not re.search('[a-z]', password):
        messages.add_message(request, constants.ERROR, 'A sua senha deve conter uma ou mais letas minúsculas')
        return False
    if not re.search('[1-9]', password):
        messages.add_message(request, constants.ERROR, 'A sua senha deve conter um ou mais números')
        return False
