...
def dados_paciente(request, id):
    ...
    if request.method == "GET":
        ...
        if not altura.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite uma altura válida')
            return redirect('/dados_paciente/')
        if not gordura.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite uma gordura válida')
            return redirect('/dados_paciente/')
        if not musculo.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite um musculo válido')
            return redirect('/dados_paciente/')
        if not hdl.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite um HDL válido')
            return redirect('/dados_paciente/')
        if not ldl.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite um LDL válido')
            return redirect('/dados_paciente/')
        if not colesterol_total.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite um colesterol válido')
            return redirect('/dados_paciente/')
        if not triglicerídios.isnumeric():
            messages.add_message(request, constants.ERROR, 'Digite um triglicerídios válido')
            return redirect('/dados_paciente/')
